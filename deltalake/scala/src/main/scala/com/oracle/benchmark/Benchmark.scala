...
abstract class Benchmark() {
  ...
  final protected lazy val spark = {
    val s = SparkSession.builder()
      ...
      .appName("Delta Lake Benchmark Simulation")
      .getOrCreate()
  ...
  private def generateJSONReport(report: BenchmarkReport, jsonReportUploadPath: String = ""): Unit = synchronized {
    import Benchmark._
    ...
    println(s"RESULT:\n$resultJson")
    uploadFile(reportLocalPath.toString, jsonReportUploadPath)
  }

  private def generateCSVReport(csvReportUploadPath: String = ""): Unit = synchronized {
  ...
  private def generateReport(): Unit = synchronized {
    val report = BenchmarkReport(
      benchmarkSpecs = "TPC-DS benchmark for Delta Lake",
