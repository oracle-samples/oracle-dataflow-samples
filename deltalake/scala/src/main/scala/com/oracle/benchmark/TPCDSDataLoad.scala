...
class TPCDSDataLoad() extends Benchmark() {
  ...
  def runInternal(): Unit = {
    ...
    runQuery(s"DROP DATABASE IF EXISTS ${dbName} CASCADE", s"base")
  ...
  def createDeltaTables(tpcdsParquetPath: String, deltaTablePath:String): Unit = {
    this.sourceLocation = tpcdsParquetPath
    this.dbLocation = deltaTablePath
     runInternal()
  }
